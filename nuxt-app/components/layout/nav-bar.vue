<template>
    <nav class="sticky top-0 z-50 p-4 bg-slate-800 text-white flex flex-row justify-between items-center">
...
<script setup lang="ts">
    ...
    // On extrait la locale courante et la fonction pour la modifier
    ...
    // menuItems devient une propriété calculée (réactive selon la langue)
    const menuItems = computed(() => [
        { label: locale.value === 'fr' ? 'Accueil' : 'Home', to: '/' },
        { label: locale.value === 'fr' ? 'CV' : 'Resume', to: '/resume' },
        { label: locale.value === 'fr' ? 'Portfolio' : 'Portfolio', to: '/portfolio' },
        { label: locale.value === 'fr' ? 'Contact' : 'Contact', to: '/contact' },
    ])
</script>
